...
5. **Is this Helm-only?** — If yes, use `hops-got-helm-pattern` agent as the entry point instead.
...
1. Delegate to **`hops-got-template-structure`** — Create directory layout, XRD definition, composition, and template files.
2. Delegate to **`hops-got-observed-state`** — Set up state-init and state-compute with $state namespace.
3. Delegate to **`hops-got-template-patterns`** — Apply the relevant patterns for the resources being managed.

### Phase 2: Cross-Cutting Concerns
4. Delegate to **`hops-xr-labels`** — Add labels/tags to schema and all resource templates.
5. Delegate to **`hops-got-status-output`** — Design status fields based on what observed values to expose.
...
### Phase 3: Refinement
8. Delegate to **`hops-got-template-simplification`** — Review and simplify the generated templates.
...
If the user indicated this is a Helm-only XRD, delegate to **`hops-got-helm-pattern`** first, which replaces Phases 1-2 with simpler patterns, then continue from Phase 2 step 4 onward.
