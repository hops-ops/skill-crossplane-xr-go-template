...
- **5 slash commands** provide common workflows (`/hops-xr-new`, `/hops-xr-validate`, `/hops-xr-checklist`, `/hops-xr-got-simplify`, `/hops-xr-audit`)
...
| `/hops-xr-got-simplify` | Simplify templates |
...
| Agent | Domain |
|-------|--------|
| `hops-xr-got-template-structure` | Template file organization, $state namespace |
| `hops-xr-got-observed-state` | Observed resource values, state-init/compute |
| `hops-xr-got-template-patterns` | forProvider, Helm values, multi-provider, PodIdentity |
| `hops-xr-got-status-output` | XRD status design, anti-corruption layer |
...
| `hops-xr-got-template-simplification` | Reducing template complexity |
| `hops-xr-got-helm-pattern` | Helm-only XRD pattern |
