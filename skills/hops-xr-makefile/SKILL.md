...
description: |
  Makefile template for XRD configurations with parallel render/validate targets,
  test execution, and build/publish commands.
---

# Makefile Template for XRD Configurations

Full template for XRD configuration Makefiles with parallel render/validate targets.
...
EXAMPLE_DEFAULT := examples/<plural>/standard.yaml
...
Replace `<name>` with the configuration name and `<plural>` with the XRD plural form. Keep the `EXAMPLES` list synchronized with GitHub Actions workflow inputs.
